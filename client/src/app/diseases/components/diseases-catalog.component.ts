...
import { DiseasesFilterFormComponent } from "./diseases-filter-form.component";
...
import { Disease, DiseaseParams, DiseasesService } from "src/app/diseases/diseases.config";

@Component({
  host: { class: 'mb-9', },
  selector: '[diseasesCatalog]',
  template: `
  <div class="pb-2">
  <h1 class="mb-4">{{ service.dictionary.Diseases.title}}</h1>
  <div>
    <div class="row align-items-center justify-content-between g-3 mb-2">
      <div class="col-12 col-md-auto">
        <a type="button" class="btn btn-primary me-4" (click)="service.clickLink(undefined, key(), 'create', 'page')">
          ...
          Crear {{ service.dictionary.Diseases.singular }}
        </a>
        ...
        <button mat-raised-button [matBadge]="count" matBadgePosition="after" (click)="service.deleteRange$(key())">
          Eliminar {{ count > 1 ? service.dictionary.Diseases.plural : service.dictionary.Diseases.singular }}
        </button>
        }
        }
      </div>
...
<div tableWrapper>
  <div tableResponsive>
    ...
    <table diseasesTable [mode]="mode()" [data]="_list" [key]="key()" [view]="view()">
    </table>
    }
  </div>
  ...
  `,
  standalone: true,
  imports: [ FontAwesomeModule, DiseasesFilterFormComponent,
    DiseasesTableComponent, CommonModule,
    RouterModule, ControlsModule, TableModule, DiseasesFilterFormComponent,
    CdkModule, MaterialModule,
   ],
})
export class DiseasesCatalogComponent implements OnInit, OnDestroy {

...
import { UserHeaderComponent } from "src/app/users/components/user-header.component";

@Component({
  selector: 'div[userNewView]',
  template: `
  <div userHeader [id]="null" [item]="null" [key]="undefined" [use]="use()" [view]="view()" (fillForm)="onFillForm()" [role]="role()"></div>
  <div userForm [use]="use()" [id]="null" [view]="view()" [role]="role()"></div>
  `,
  standalone: true,
  imports: [ UserFormComponent, ModalWrapperModule, UserHeaderComponent, ],
})
...
\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
  ...
  `,
  standalone: true,
  imports: [UserFormComponent, UserHeaderComponent],
})
...
@Component({
  ...
  template: `
  <div userHeader [id]="id()" [item]="item()" [key]="key()" [use]="use()" [view]="view()" [role]="role()"></div>
  <div userForm [use]="use()" [id]="id()" [view]="view()" [role]="role()"></div>
  `,
  standalone: true,
  imports: [ UserFormComponent, ModalWrapperModule, UserHeaderComponent, ],

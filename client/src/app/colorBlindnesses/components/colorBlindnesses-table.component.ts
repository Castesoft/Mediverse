...
import {
  ColorBlindness,
  ColorBlindnessParams,
  ColorBlindnessesService,
} from 'src/app/colorBlindnesses/colorBlindnesses.config';
  ...
  template: `
    <div class="dropdown-menu d-block" cdkMenu>
      <a
        ...
        [href]="service.dictionary.ColorBlindnesses.catalogRoute + '/' + item().id"
        (click)="
          service.clickLink(item(), key(), 'detail', 'page');
          $event.preventDefault()
        "
      >
        Ver {{ service.dictionary.ColorBlindnesses.singular }}
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        ...
        "
      >
        Abrir {{ service.dictionary.ColorBlindnesses.singular }} en pantalla modal
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        [routerLink]="[service.dictionary.ColorBlindnesses.catalogRoute, item().id, 'editar']"
      >
        Editar
      </a>
      <button
        ...
        (click)="service.delete$(item(), 'ColorBlindnesses')"
      >
        Eliminar
      </button>
    </div>
...
{
  item: InputSignal<ColorBlindness> = input.required();
...
@Component({
  ...
  template: `
    <thead
      tableHeader
      [columns]="service.columns['ColorBlindnesses']"
      [params]="params"
      ...
      @for (item of data(); track item.id; let idx = $index) {
        ...
        >
          <td
            tableCheckCell
            [idx]="idx"
            [dictionary]="service.dictionary.ColorBlindnesses"
          ...
          >
            <div class="d-flex align-items-center justify-content-start px-0">
              <a
                [routerLink]="[service.dictionary.ColorBlindnesses.catalogRoute, item.id]"
                [href]="[service.dictionary.ColorBlindnesses.catalogRoute, item.id]"

...
@Component({
  selector: 'div[occupationsTableMenu]',
  host: { class: '' },
  template: `
...
export class OccupationsTableMenuComponent
...
@Component({
  ...
  selector: 'table[occupationsTable]',
  template: `
    <thead
      tableHeader
      ...
      @for (item of data(); track item.id; let idx = $index) {
        ...
        >
          ...
          <ng-template #context_menu>
            <div occupationsTableMenu [item]="item" [key]="key()"></div>
          </ng-template>
        </tr>
      }
    </tbody>
  ...
  imports: [
    ...
    MaterialModule,
    CommonModule,
    OccupationsTableMenuComponent,
  ],
})
export class OccupationsTableComponent implements OnInit, OnDestroy {
  ...
  cuid = createId();
  selected = false;
  row: TableRow<Occupation> = new TableRow<Occupation>(new Occupation());

  cells: PartialCellsOf<Occupation> = {
  ...
  constructor() {
    effect(() => {
      this.params = new OccupationParams(this.key());
...
@Component({
  selector: '[occupationsFilterForm]',
  template: `
  @if(role() === 'compact') {
...
export class OccupationsFilterFormComponent extends FormComponent<OccupationsService> implements OnInit, FilterFormGroupActions<Occupation, OccupationParams, FormGroup2<OccupationParams>> {
  ...
  private ngUnsubscribe = new Subject<void>();
  params!: OccupationParams;
  info: FormInfo<OccupationParams> = {
  ...
  form: FormGroup2<OccupationParams> = new FormGroup2<OccupationParams>(OccupationParams as any, new OccupationParams(createId()), this.info, { orientation: 'inline', use: 'filter' });
  ...
  onSubmit() {
    ...
    // this.service.submitForm(this.key(), result as OccupationParams);
  }
}


@Component({
  selector: '[occupationsCatalog]',
  template: `
  <div class="pb-2">
...
<div tableWrapper>
  <div tableResponsive>
    ...
    <table occupationsTable [mode]="mode()" [data]="_list" [key]="key()" [view]="view()">
    </table>
    }
  </div>
  ...
  `,
  standalone: true,
  imports: [ FontAwesomeModule, OccupationsFilterFormComponent,
    OccupationsTableComponent, CommonModule,
    RouterModule, ControlsModule, TableModule, OccupationsFilterFormComponent,
    CdkModule, MaterialModule,
   ],
})
export class OccupationsCatalogComponent implements OnInit, OnDestroy {
  service = inject(OccupationsService);
  icons = inject(IconsService);
...
export class OccupationsService extends ServiceHelper<Occupation, OccupationParams, FormGroup2<OccupationParams>> {
  ...
  clickLink = (item: Occupation | undefined = undefined, key: string | undefined = undefined,

...
import {
  ...
  OccupationParams,
  OccupationsService,
} from 'src/app/occupations/occupations.config';
  ...
  template: `
    <div class="dropdown-menu d-block" cdkMenu>
      <a
        ...
        [href]="service.dictionary.Occupations.catalogRoute + '/' + item().id"
        (click)="
          service.clickLink(item(), key(), 'detail', 'page');
          $event.preventDefault()
        "
      >
        Ver {{ service.dictionary.Occupations.singular }}
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        ...
        "
      >
        Abrir {{ service.dictionary.Occupations.singular }} en pantalla modal
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        [routerLink]="[service.dictionary.Occupations.catalogRoute, item().id, 'editar']"
      >
        Editar
      </a>
      <button
        ...
        (click)="service.delete$(item(), 'Occupations')"
      >
        Eliminar
      </button>
    </div>
...
@Component({
  ...
  template: `
    <thead
      tableHeader
      [columns]="service.columns['Occupations']"
      [params]="params"
      ...
      @for (item of data(); track item.id; let idx = $index) {
        ...
        >
          <td
            tableCheckCell
            [idx]="idx"
            [dictionary]="service.dictionary.Occupations"
          ...
          >
            <div class="d-flex align-items-center justify-content-start px-0">
              <a
                [routerLink]="[service.dictionary.Occupations.catalogRoute, item.id]"
                [href]="[service.dictionary.Occupations.catalogRoute, item.id]"

...
@Component({
  ...
  template: `
    <div class="card-header">
      ...
      <div class="card-toolbar">
        <button createBtn (click)="service.clickLink(null, null, key(), 'create', 'modal')"
  ...
  imports: [ BsDropdownModule, RouterModule, ReactiveFormsModule, FontAwesomeModule, DecimalPipe,
    ...
    LayoutModule, EventsFilterMenuComponent, LayoutModule, FullCalendarModule,
  ],
})
export class EventsCalendarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.service.loading$(this.key())
      ...
      this.calendarOptions = {
        plugins: [dayGridPlugin, interactionPlugin, timeGridDayPlugin, timeGridWeekPlugin],
        selectable: true,
        editable: true,
        headerToolbar: {
  ...
  handleDateClick(arg: DateClickArg) {
    this.service.clickLink(null, null, this.key(), 'create', 'modal', arg.date, arg.date);
  }
  ...
  private handleSelect(arg: DateSelectArg) {
    if(calcDateDiff(arg.start, arg.end) !== -1) {
      this.service.clickLink(null, null, this.key(), 'create', 'modal', arg.start, arg.end);
    }
  }

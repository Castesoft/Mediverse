...
@Component({
  selector: 'div[educationLevelsTableMenu]',
  host: { class: '' },
  template: `
...
export class EducationLevelsTableMenuComponent
...
@Component({
  ...
  selector: 'table[educationLevelsTable]',
  template: `
    <thead
      tableHeader
      ...
      @for (item of data(); track item.id; let idx = $index) {
        ...
        >
          ...
          <ng-template #context_menu>
            <div educationLevelsTableMenu [item]="item" [key]="key()"></div>
          </ng-template>
        </tr>
      }
    </tbody>
  ...
  imports: [
    ...
    MaterialModule,
    CommonModule,
    EducationLevelsTableMenuComponent,
  ],
})
export class EducationLevelsTableComponent implements OnInit, OnDestroy {
  ...
  cuid = createId();
  selected = false;
  row: TableRow<EducationLevel> = new TableRow<EducationLevel>(new EducationLevel());

  cells: PartialCellsOf<EducationLevel> = {
...
@Component({
  selector: '[educationLevelsFilterForm]',
  template: `
  @if(role() === 'compact') {
...
export class EducationLevelsFilterFormComponent extends FormComponent<EducationLevelsService> implements OnInit, FilterFormGroupActions<EducationLevel, EducationLevelParams, FormGroup2<EducationLevelParams>> {
  ...
  private ngUnsubscribe = new Subject<void>();
  params!: EducationLevelParams;
  info: FormInfo<EducationLevelParams> = {
  ...
  form: FormGroup2<EducationLevelParams> = new FormGroup2<EducationLevelParams>(EducationLevelParams as any, new EducationLevelParams(createId()), this.info, { orientation: 'inline', use: 'filter' });
  ...
  onSubmit() {
    ...
    // this.service.submitForm(this.key(), result as EducationLevelParams);
  }
}
...
@Component({
  selector: "div[educationLevelForm]",
  template: `
  <form [id]="form.id" (ngSubmit)="onSubmit()">
...
export class EducationLevelFormComponent extends FormComponent<EducationLevelsService> implements OnInit, FormGroupActions<EducationLevel, FormGroup2<EducationLevel>> {
  ...
  form: FormGroup2<EducationLevel> = new FormGroup2<EducationLevel>(EducationLevel, new EducationLevel(), this.info, { orientation: 'inline', use: 'create' });
...
@Component({
  selector: 'div[educationLevelDetail]',
  template: `
  <div container [type]="'inline'">
  ...
  <div educationLevelForm [item]="item()" [key]="key()" [use]="use()" [view]="view()"></div>
  `,
  standalone: true,
  imports: [EducationLevelFormComponent, ControlsModule,],
})
...
@Component({
  selector: 'educationLevel-detail-modal',
  template: `
    <div modalContent>
      @if (title) {
...
export class EducationLevelsService extends ServiceHelper<EducationLevel, EducationLevelParams, FormGroup2<EducationLevelParams>> {
  constructor() {
    super(EducationLevelParams, 'educationLevels', new NamingSubject(
      'feminine',
      'especialidad',
      'especialidades',
      'Especialidades',
  ...
  clickLink = (item: EducationLevel | undefined = undefined, key: string | undefined = undefined,

...
} from 'src/app/educationLevels/educationLevels.config';
  ...
  template: `
    <div class="dropdown-menu d-block" cdkMenu>
      <a
        ...
        [href]="service.dictionary.EducationLevels.catalogRoute + '/' + item().id"
        (click)="
          service.clickLink(item(), key(), 'detail', 'page');
          $event.preventDefault()
        "
      >
        Ver {{ service.dictionary.EducationLevels.singular }}
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        ...
        "
      >
        Abrir {{ service.dictionary.EducationLevels.singular }} en pantalla modal
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        [routerLink]="[service.dictionary.EducationLevels.catalogRoute, item().id, 'editar']"
      >
        Editar
      </a>
      <button
        ...
        (click)="service.delete$(item(), 'EducationLevels')"
      >
        Eliminar
      </button>
    </div>
...
@Component({
  ...
  template: `
    <thead
      tableHeader
      [columns]="service.columns['EducationLevels']"
      [params]="params"
      ...
      @for (item of data(); track item.id; let idx = $index) {
        ...
        >
          <td
            tableCheckCell
            [idx]="idx"
            [dictionary]="service.dictionary.EducationLevels"
          ...
          >
            <div class="d-flex align-items-center justify-content-start px-0">
              <a
                [routerLink]="[service.dictionary.EducationLevels.catalogRoute, item.id]"
                [href]="[service.dictionary.EducationLevels.catalogRoute, item.id]"

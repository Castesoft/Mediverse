...
import { EducationLevelsService, EducationLevel } from "src/app/educationLevels/educationLevels.config";
import { FormGroup2, FormInfo } from "src/app/_forms/form2";
import { FormNewModule } from "src/app/_forms/_new/forms-new.module";

@Component({
  selector: "[educationLevelForm]",
  template: `
  ...
  @if(use() !== 'detail') {
    <div container [type]="'inline'">
      <div detailFooter [use]="use()" [view]="view()" [id]="item() ? item()!.id : undefined" [dictionary]="service.dictionary.EducationLevels" [formId]="form.id"></div>
    </div>
  }
</form>
...
export class EducationLevelFormComponent extends FormComponent<EducationLevelsService> implements OnInit, FormGroupActions<EducationLevel, FormGroup2<EducationLevel>> {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (value) {
        this.form = new FormGroup2<EducationLevel>(EducationLevel, value, this.info, { orientation: 'inline' });
...
@Component({
  ...
  template: `
  <div container [type]="'inline'">
    <div detailHeader [(use)]="use" [view]="view()" [dictionary]="service.dictionary.EducationLevels" [id]="item() ? item()!.id : undefined" (onDelete)="service.delete$(item()!, 'EducationLevels')"></div>
  </div>

...
> {
  ...
  constructor(
    ...
    entityInstance: new (init?: Partial<T>) => T,
    options?: { tableCells?: PartialCellsOf<T> }
  ) {
    this.service = inject(serviceToken);
    this.row.set(new TableRow<T>(new entityInstance));

    if (options?.tableCells !== undefined) {
      this.cells.set(options.tableCells);
    } else {
      this.cells.set({ ...baseTableCells } as PartialCellsOf<T>);
    }
  }
}

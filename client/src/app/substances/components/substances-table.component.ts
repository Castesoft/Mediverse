...
import {
  ...
  SubstanceParams,
  SubstancesService,
} from 'src/app/substances/substances.config';
  ...
  template: `
    <div class="dropdown-menu d-block" cdkMenu>
      <a
        ...
        [href]="service.dictionary.Substances.catalogRoute + '/' + item().id"
        (click)="
          service.clickLink(item(), key(), 'detail', 'page');
          $event.preventDefault()
        "
      >
        Ver {{ service.dictionary.Substances.singular }}
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        ...
        "
      >
        Abrir {{ service.dictionary.Substances.singular }} en pantalla modal
      </a>
      <a
        cdkMenuItem
        class="dropdown-item px-3"
        [routerLink]="[service.dictionary.Substances.catalogRoute, item().id, 'editar']"
      >
        Editar
      </a>
      <button
        ...
        (click)="service.delete$(item(), 'Substances')"
      >
        Eliminar
      </button>
    </div>
...
@Component({
  ...
  template: `
    <thead
      tableHeader
      [columns]="service.columns['Substances']"
      [params]="params"
      ...
      @for (item of data(); track item.id; let idx = $index) {
        ...
        >
          <td
            tableCheckCell
            [idx]="idx"
            [dictionary]="service.dictionary.Substances"
          ...
          >
            <div class="d-flex align-items-center justify-content-start px-0">
              <a
                [routerLink]="[service.dictionary.Substances.catalogRoute, item.id]"
                [href]="[service.dictionary.Substances.catalogRoute, item.id]"

...
@Component({
  selector: 'div[substancesTableMenu]',
  host: { class: '' },
  template: `
...
export class SubstancesTableMenuComponent
...
@Component({
  ...
  selector: 'table[substancesTable]',
  template: `
    <thead
      tableHeader
      ...
      @for (item of data(); track item.id; let idx = $index) {
        ...
        >
          ...
          <ng-template #context_menu>
            <div substancesTableMenu [item]="item" [key]="key()"></div>
          </ng-template>
        </tr>
      }
    </tbody>
  ...
  imports: [
    ...
    MaterialModule,
    CommonModule,
    SubstancesTableMenuComponent,
  ],
})
export class SubstancesTableComponent implements OnInit, OnDestroy {
  ...
  cuid = createId();
  selected = false;
  row: TableRow<Substance> = new TableRow<Substance>(new Substance());

  cells: PartialCellsOf<Substance> = {
  ...
  constructor() {
    effect(() => {
      this.params = new SubstanceParams(this.key());
...
export class SubstancesFilterFormComponent extends FormComponent<SubstancesService> implements OnInit, FilterFormGroupActions<Substance, SubstanceParams, FormGroup2<SubstanceParams>> {
  ...
  form: FormGroup2<SubstanceParams> = new FormGroup2<SubstanceParams>(SubstanceParams as any, new SubstanceParams(createId()), this.info, { orientation: 'inline', use: 'filter' });
  ...
  onSubmit() {
    ...
    // this.service.submitForm(this.key(), result as SubstanceParams);
  }
}


@Component({
  selector: '[substancesCatalog]',
  template: `
  <div class="pb-2">
...
<div tableWrapper>
  <div tableResponsive>
    ...
    <table substancesTable [mode]="mode()" [data]="_list" [key]="key()" [view]="view()">
    </table>
    }
  </div>
  ...
  `,
  standalone: true,
  imports: [ FontAwesomeModule, SubstancesFilterFormComponent,
    SubstancesTableComponent, CommonModule,
    RouterModule, ControlsModule, TableModule, SubstancesFilterFormComponent,
    CdkModule, MaterialModule,
   ],
})
export class SubstancesCatalogComponent implements OnInit, OnDestroy {
  service = inject(SubstancesService);
  icons = inject(IconsService);
...
export class SubstancesService extends ServiceHelper<Substance, SubstanceParams, FormGroup2<SubstanceParams>> {
  constructor() {
    super(SubstanceParams, 'substances', new NamingSubject(
      'feminine',
      'sustancia',
      'sustancias',
      'Sustancias',
  ...
  clickLink = (item: Substance | undefined = undefined, key: string | undefined = undefined,
...
@NgModule({
  imports: [RouterModule.forChild([
    {
      path: '', title: 'Sustancias', data: { breadcrumb: 'Sustancias', },
      component: SubstancesComponent, runGuardsAndResolvers: 'always',
      children: [
        { path: '', component: CatalogComponent, title: 'Catálogo de sustancias', data: { breadcrumb: 'Catálogo', }, },
        { path: 'nuevo', component: NewComponent, title: 'Crear nueva sustancia', data: { breadcrumb: 'Nuevo', }, },
        {

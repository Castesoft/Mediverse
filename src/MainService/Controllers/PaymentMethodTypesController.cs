...
public class PaymentMethodTypesController(IUnitOfWork uow, IMapper mapper, IPaymentMethodTypesService service) : BaseApiController
{
    private static readonly string EntityName = "Tipo de Método de Pago";
    
    [HttpGet]
    public async Task<ActionResult<PagedList<PaymentMethodTypeDto>>> GetPagedListAsync([FromQuery] PaymentMethodTypeParams param)
    {
        var pagedList = await uow.PaymentMethodTypeRepository.GetPagedListAsync(param);
    ...
    public async Task<ActionResult<PaymentMethodTypeDto>> ToggleVisibilityAsync([FromRoute] int id)
    {
        ...
        return await uow.PaymentMethodTypeRepository.GetDtoByIdAsync(id);
    }

    [HttpGet("options")]
    public async Task<ActionResult<List<OptionDto>>> GetOptionDtosAsync() =>
        await uow.PaymentMethodTypeRepository.GetOptionsAsync();

    [HttpGet("all")]
    public async Task<ActionResult<List<PaymentMethodTypeDto>>> GetAllAsync()
    {
        var data = await uow.PaymentMethodTypeRepository.GetAllDtosAsync();
    ...
    public async Task<ActionResult<PaymentMethodTypeDto>> AddAsync([FromBody] PaymentMethodTypeCreateDto request)
    {
        ...
        if (!await uow.PaymentMethodTypeRepository.ExistsByCodeAsync(request.Code))
            return BadRequest($"El código {request.Code} ya existe.");

        PaymentMethodType itemToAdd = new();

        mapper.Map<PaymentMethodTypeCreateDto, PaymentMethodType>(request, itemToAdd);

        uow.PaymentMethodTypeRepository.Add(itemToAdd);

        if (!await uow.Complete()) return BadRequest($"Error al agregar {EntityName}.");

        return await uow.PaymentMethodTypeRepository.GetDtoByIdAsync(itemToAdd.Id);
    }
    ...
    {
        var itemToUpdate = await uow.PaymentMethodTypeRepository.GetByIdAsync(id);

        if (itemToUpdate == null) return NotFound($"{EntityName} con ID {id} no encontrado.");

        mapper.Map<PaymentMethodTypeUpdateDto, PaymentMethodType>(request, itemToUpdate);
    ...
    {
        if (!await uow.PaymentMethodTypeRepository.ExistsByIdAsync(id)) return BadRequest($"{EntityName} con ID {id} no existe.");
    ...
    [HttpGet("xlsx")]
    public async Task<ActionResult> ExportExcelAsync([FromQuery] PaymentMethodTypeParams param)
    {
        PagedList<PaymentMethodTypeDto> data = await uow.PaymentMethodTypeRepository.GetPagedListAsync(param, true);
        List<PaymentMethodTypeDto> dataToExport = mapper.Map<List<PaymentMethodTypeDto>>(data);
